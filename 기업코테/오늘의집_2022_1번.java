...
public class 오늘의집_2022_1번 {
    public String[] solution(String path) {
        List<String> result = new ArrayList<>();
        int time = 0;
        boolean[] checked = new boolean[path.length()];
        for (int i = 0; i < path.length(); i++) {
            if (checked[i]) continue;
            checked[i] = true;
            char start = path.charAt(i);
            int distance = 1;
            String direction = "";
            for (int j = i + 1; j < path.length(); j++) {
                char next = path.charAt(j);
                if (start == next) {
                    distance++;
                } else {
                    if (start == 'E') {
            ...
            result.add("Time " + time + ": Go straight " + (distance * 100) + "m and turn " + direction);
            time += distance;
        }
